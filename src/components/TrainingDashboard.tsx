...
const PROGRAM_PREVIEWS: Record<string, { format: string; level: string; focus: string; access: string }> = {
  'individual-basic': {
    format: '10 занятий + ДЗ',
    level: 'с нуля',
    focus: 'базовая программа обучения',
  ...
  'individual-advanced': {
    format: '14 занятий + ДЗ',
    level: 'после базы',
    focus: 'расширенная программа обучения',
    access: 'подробный план после оплаты',
  },
};

const buildProgramCurriculum = (count: number) =>
  Array.from({ length: count }, (_, index) => ({
    title: `Занятие ${index + 1}`,
    focus: 'Тема и практика занятия будут добавлены в программу курса.',
  }));

const PROGRAM_CURRICULUM: Record<string, Array<{ title: string; focus: string }>> = {
  'individual-basic': buildProgramCurriculum(10),
  'group-basic': buildProgramCurriculum(10),
  'individual-advanced': buildProgramCurriculum(14),
};
